...
ytt -f templates/build --data-value-yaml metadata_name=${APP_NAME}-${APP_IMAGE_REVISION} --data-value-yaml git_revision=$APP_IMAGE_REVISION --data-value-yaml git_url=$APP_GIT_URL --data-value-yaml tag=${APP_IMAGE_TAG}:${APP_IMAGE_REVISION} -o yaml > tbs/spring-petclinic-image.yaml
